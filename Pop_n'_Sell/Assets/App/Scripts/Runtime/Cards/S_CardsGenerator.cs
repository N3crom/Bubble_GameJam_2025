...
public class S_CardsGenerator : MonoBehaviour
{
    [Header("Parameter")]
    ...
    void GenerateCards(S_ClassCustomer currentCustomer)
    {
        ...
        if (_ssoItemsList.Value.Count < _rsoNumberItemsDisplay.NumberItemsToDisplay)
        {
        ...
        foreach (var element in _ssoItemsList.Value)
        {
            if (!_rsoItemsListToDisplay.Value.Contains(element))
            {
                _rsoItemsListToDisplay.Value.Add(element);
                numberCards++;
        ...
        ShuffleList(_rsoItemsListToDisplay.Value);
    }

    void SpawnCards()
    {
        _rseOnListGenerationFinish.Call(_rsoItemsListToDisplay.Value);
